...
def questions_messages(role: str, seniority: str, count: int) -> list[dict]:
    return [
        {"role": "system", "content": QUESTIONS_SYSTEM.format(role=role, seniority=seniority, count=count)},
        {"role": "user", "content": "Generate the questions now."},
    ]


def scoring_messages(role: str, seniority: str, question: str, answer: str) -> list[dict]:
    return [
        {"role": "system", "content": SCORING_SYSTEM.format(
            role=role, seniority=seniority, question=question, answer=answer)},
